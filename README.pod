...
Every swat story comes with some settings you may define to alter story execution. One of the usual cases add http data when making POST or PUT requests.

These type of settings could be done at swat ini files.

Swat ini files are file called "swat.ini" and located at resources directory. Swat.ini file defines additional settings for a requested http resourse:
...
All swat ini variables could be devided on two groups:
...
Coomon settings are way to adjust the behaviour/output of swat test runner.
...
C<ignore_http_err> - ignore http errors, if this parameters is off (set to C<1>) I<error http codes> in response will not result in test fails,
