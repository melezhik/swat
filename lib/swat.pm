...
Swat uses L<prove utility|https://metacpan.org/pod/distribution/Test-Harness/bin/prove> to run tests.



=item *

If C<--prove> parameter is not set swat runs C<prove> with C<-r --verbose> options. 



=item *

If user redefines the options via C<--prove> parameter, swat runs prove with options comes from C<--prove>.
...
Unless user sets path to configuration file explicitly by C<--ini> or C<--yaml> swat runner looks for the 
files named suite.ini and I<then> ( if not found ) looks for the file named suite.yaml at the current working directory.
